...
pub fn add(
    ...
    // C imports for locale constants and functions
    {
        ...
        if (target.result.os.tag.isDarwin()) {
            const sys_include_dir = if (b.graph.environ_map.get("SDKROOT")) |sdk|
                try std.fs.path.join(b.allocator, &.{ sdk, "usr", "include" })
            else blk: {
                ...
                break :blk libc.sys_include_dir.?;
            };
            c.addSystemIncludePath(.{ .cwd_relative = sys_include_dir });
        }
    ...
    // OpenGL libraries need their own GLAD implementation because the host
    // supplies only a context and procedure lookup callback.
